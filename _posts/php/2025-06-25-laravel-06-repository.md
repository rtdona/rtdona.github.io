...
2. [Repository의 필요성](#2-repository의-필요성)
...
비즈니스 로직과 데이터 접근 로직을 분리하여, 코드가 훨씬 깔끔하고 유지보수하기 쉬워집니다.

### 2.2. 데이터 추상화(Data Abstraction)
DB, 파일, 외부 API 등 데이터 소스가 바뀌어도 코드 수정이 최소화됩니다.  
컨트롤러/서비스는 데이터 저장 방식(Eloquent Orm, 쿼리 등)을 알 필요 없이 Repository의 메서드만 호출하여 사용할 수 있습니다.

### 2.3. 테스트 용이성
Repository를 Mocking 으로 대체하여 테스트 코드를 작성하기 편리해지고,  
외부 의존성에 영향 받지 않고 유닛 테스트를 가능하게 합니다.

### 2.4. 코드 일관성
프로젝트 내에 여러 곳에서 같은 쿼리/데이터 접근이 필요하더라도 Repository에서만 관리하면 되기 때문에,  
코드가 중복되는 것을 방지하고, 업데이트 일원화가 가능해집니다.

### 2.5. 확장성과 유연성
비즈니스 요구로 데이터 소스가 변경되더라도(예: MySQL → MongoDB) Repository 구현체만 수정하면 되니,  
전체 애플리케이션 구조는 그대로 유지할 수 있습니다.
